...
if($_SERVER["REQUEST_METHOD"] == "POST" AND $table=='AdminUsers'){
	...
	$sql = " INTO {$table} (username, passcode)
	VALUES ('{$name}', '{$passcode}')";
	$sql = "UPDATE AdminUsers SET name='{$name}', password='{$passcode}' WHERE id={$_GET['id']}";
...
if($_SERVER["REQUEST_METHOD"] == "POST" AND $table=='Scarfvesdb'){
	...
	$image = $_POST['image']; 
	$sql = " INTO {$table} (description, price, availability, image)
	VALUES ('{$description}', '{$price}','{$available}','{$image}')";
	$sql = "UPDATE Scarfvesdb SET description='{$description}', price='{$price}', availability = '{$available}', image = '{$image}' WHERE ScarfID={$_GET['ScarfID']}";
	$sql = "UPDATE Scarfvesdb SET description='{$description}', price='{$price}', availability = '{$available}', image = '{$image}' WHERE ScarfID={$_GET['ScarfID']}";
	$conn->prepare($sql)->execute();
	$conn->exec($sql);
...
if(!empty($_SESSION['login_user'])){ ?>
	...
	<?php if($table=='AdminUsers'){ 
		require_once('conn.php') ;
		$data = $conn->query("SELECT * FROM AdminUsers where id='{$_GET['id']}'")->fetchAll();
		?>
			...
			<div class="main-content">
				<div class="section__content section__content--p30">	
					<form class="form-horizontal"  action="" method="post">		
						...
						<div class="control-group "><label class="control-label required" for="Aboutus_vision">User Name <span class="required">*</span></label><div class="controls"><input value="<?php echo $data[0]['name'] ?>" class="span5" maxlength="255" name="username" id="Aboutus_vision" type="text"></div></div>
						<div class="control-group "><label class="control-label required" for="Aboutus_mission">Password <span class="required">*</span></label><div class="controls"><input value="<?php echo $data[0]['password'] ?>" class="span5" maxlength="255" name="passcode" id="Aboutus_mission" type="text"></div></div>								
